...
public class GamePanel extends JPanel implements Runnable {
    ...
    public GamePanel(String playerName, Team team, String mapResource) {
        ...
        // Spawn positions: RED on left, BLUE on right; Y spread across 5 slots by name hash
        double spawnX = (team == Team.RED)
                ? 3 * GameMap.TILE_SIZE
                : (gameMap.getCols() - 4) * GameMap.TILE_SIZE;
        double spawnY = spawnYForPlayer(playerName, gameMap.getHeightPixels());
    ...
    private void spawnPowerUps() {
        ...
        powerUps.add(new PowerUp(12 * ts + ts / 2.0, 7 * ts + ts / 2.0, PowerUp.Type.HEALTH));
        powerUps.add(new PowerUp( 5 * ts + ts / 2.0, 2 * ts + ts / 2.0, PowerUp.Type.SPEED));
        powerUps.add(new PowerUp(18 * ts + ts / 2.0, 2 * ts + ts / 2.0, PowerUp.Type.AMMO));
        powerUps.add(new PowerUp( 5 * ts + ts / 2.0,12 * ts + ts / 2.0, PowerUp.Type.IMMUNITY));
        powerUps.add(new PowerUp(18 * ts + ts / 2.0,12 * ts + ts / 2.0, PowerUp.Type.SPEED));
        powerUps.add(new PowerUp( 8 * ts + ts / 2.0, 6 * ts + ts / 2.0, PowerUp.Type.AMMO));
        powerUps.add(new PowerUp(15 * ts + ts / 2.0, 6 * ts + ts / 2.0, PowerUp.Type.IMMUNITY));
        powerUps.add(new PowerUp( 8 * ts + ts / 2.0, 8 * ts + ts / 2.0, PowerUp.Type.HEALTH));
        powerUps.add(new PowerUp(15 * ts + ts / 2.0, 8 * ts + ts / 2.0, PowerUp.Type.HEALTH));
    }
    ...
    private void update() {
        ...
        if (!localTank.isAlive() && keyHandler.respawn) {
            double spawnX = (localTank.getTeam() == Team.RED)
                    ? 3 * GameMap.TILE_SIZE
                    : (gameMap.getCols() - 4) * GameMap.TILE_SIZE;
            localTank.respawn(spawnX, spawnYForPlayer(localTank.getPlayerId(), gameMap.getHeightPixels()));
        ...
        if (localTank.isAlive()) {
            ...
            // Revert if hitting a wall or another tank
        ...
        // Power-ups: respawn timers + collection
    ...
    private boolean collidesWithTanks(Tank tank) {
        ...
        for (Tank remote : remoteTanks.values()) {
            ...
            if (bounds.intersects(remote.getBounds())) return true;
        }
        return false;
    }
    ...
    private static double spawnYForPlayer(String playerId, int mapHeightPixels) {
        int slot = Math.abs(playerId.hashCode()) % 5;
        return mapHeightPixels / 2.0 + (slot - 2) * GameMap.TILE_SIZE * 2;
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        hud.draw(g2, localTank, remoteTanks, redScore, blueScore, mw, mh,
                speedUntil, immunityUntil, ammoUntil);
